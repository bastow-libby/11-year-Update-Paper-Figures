...
'''
def load_simulation_ns(input_directory):
    ...
    for j in range(len(fnum)):
        # Read file
        f = h5py.File(input_directory + '/l3_{}.hdf5'.format(fnum[j]), 'r')
        # Save stations
        ...
        mcw[np.isinf(mcw)] = 0
        mcw[np.isnan(mcw)] = 0
        mczen = np.array(np.rad2deg(f['MCPrimary']['zenith']))
        mcaz = np.array(np.rad2deg(f['MCPrimary']['azimuth']))
        index_0 = np.argwhere(np.isnan(mczen))
        index_1 = np.argwhere(np.isnan(mcaz))
        weights = np.delete(np.delete(mcw, index_0), index_1)
        stations = np.delete(np.delete(mcw, index_0), index_1)
...
if __name__ == '__main__':
    ...
    if args.stations:
        """
        bin_ends = np.linspace(0, 100, 101)
        # Bin burnsample with the energies
        data = []
        tiers = apply_cuts(stations, stations, bins)
        for tier in tiers:
            dat_err = bin_weighted_param(tier, bin_ends)[-1]
            data.append([tier, dat_err])
        ...
        simulations = []
        for ind, reco in reco_key:
            sim_nstat, sim_weights = load_simulation(args.sim_directory, "Stations", "T{}".format(ind), reco)
            err = bin_weighted_param(sim_ns, bin_ends, sim_weights)[-1]
            simulations.append([sim_ns, sim_weights, err])

        if args.ratio:
            ratio_sim_data_all_tiers(data, simulations, "NStations", bin_ends,
                                     reco_key, logscale=args.log, output=args.output+'/ratio-nstation.png')
        else:
            dist_sim_data_all_tiers(data, simulations, "NStations", bin_ends,
                                    reco_key, logscale=args.log, output=args.output+'/simvdat-nstation.png')
        """
        
        bin_ends = np.arange(5, 22, 1)
        ...
        plt.savefig('./welp.png', transparent=False)
        ...
        if args.ratio:
            ...
            dist_sim_data_all_tiers(data, simulations, "NStations", bin_ends,
                                    reco_key, logscale=args.log, output=args.output+'/simvdat-nstation.png')
        """
    if len(args.stations_sim) > 0:
        yearly = [2012, 2015, 2018]
        bin_ends = np.arange(5, 22, 1)
        #bin_centres = (bin_ends[1:] + bin_ends[:-1])/2
        bin_centres = bin_ends[:-1]
        for i, dirc in enumerate(args.stations_sim):
            sim_nstat = load_simulation(dirc, "Stations", "T1", "MC")[0]
            sim_weights = []
            tiers = apply_cuts(sim_nstat, sim_nstat, bins, radians=False)
            for ind, reco in reco_key:
                sim_weights.append(load_simulation(dirc, "Stations", "T{}".format(ind), "MC")[-1])
            ...
            err = bin_weighted_param(sim_ns, bin_ends, sim_w)[-1]
            y_reco = np.histogram(sim_ns, bin_ends, weights=sim_w)[0]
            plt.errorbar(bin_centres, y_reco/sum(y_reco), yerr=err/sum(y_reco), label=yearly[i], capsize=3)
