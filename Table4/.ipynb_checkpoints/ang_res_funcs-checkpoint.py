...
def plot_ang_res_by_tier(data_list, bins_list, tiers, titles, param, x_coord):
    ...
    for data, bins, tier, title in zip(data_list, bins_list,
        ...
        s2 = 'Median: {}'.format(round(np.median(data),3))
        ...
        s3 = '68%: {} - {}'.format(round(median - low_err, 3), round(high_err - median, 3))
        print (s3)
...
def zenith_diff(true_zen, reco_zen, bin_ends, cut=95):
    print(cut)
    # Zenith difference
...
def ratio_sim_data_all_tiers(data_list, reco_list, param, bin_ends, titles, logscale=False, output='./plot.png'):
    ...
    plt.savefig(output, transparent=True)
#Uses true zenith
def zenith_diff_corrected2(true_zen, reco_zen, bin_ends, cut=65):
    # Use TRUE zenith for binning AND cuts
    mask = true_zen < cut  # Apply cut on TRUE zenith
    y = np.abs(reco_zen - true_zen)[mask]
    x = true_zen[mask]  # Bin by TRUE zenith
    
    bins = (bin_ends[1:] + bin_ends[:-1]) / 2
    xerr = (bin_ends[1:] - bin_ends[:-1]) / 2

    binnum = np.digitize(x, bin_ends, right=False) - 1
    dzen_binned = [[] for i in range(len(bins))]
    for i in range(len(y)):
        if binnum[i] < len(dzen_binned):  # Safety check
            dzen_binned[binnum[i]].append(y[i])
    ...
    return [yave, xerr, yerr, bins]
#Angular difference
def open_angle_vs_zen_corrected(true_zen, reco_zen, true_az, reco_az, bin_ends, cut=65):
    opening_angle = find_opening_angle(true_zen, reco_zen, true_az, reco_az)
    
    # Use TRUE zenith for binning AND cuts
    mask = true_zen < cut
    y = opening_angle[mask]
    x = true_zen[mask]  # Bin by TRUE zenith
    ...
    binnum = np.digitize(x, bin_ends, right=False) - 1
    opening_binned = [[] for i in range(len(bins))]
    for i in range(len(y)):
        if binnum[i] < len(opening_binned):  # Safety check
            opening_binned[binnum[i]].append(y[i])

    yave = []
    yler = []
    yher = []
    for b in opening_binned:
        if len(b) > 0:
            yave.append(np.median(b))
            yler.append(np.quantile(b, .16))
            yher.append(np.quantile(b, .84))
...
def new_zenith_diff_p_fe_by_tier(mc_zen_p, reco_zen_p, mc_zen_fe, reco_zen_fe,
                             bin_ends, titles, cuts):
    data = []
    for (true_prot_zen, reco_prot_zen, true_fe_zen, reco_fe_zen, bin_end, cut) \
            in zip(mc_zen_p, reco_zen_p, mc_zen_fe, reco_zen_fe, bin_ends, cuts):
        proton = zenith_diff_corrected2(true_prot_zen, reco_prot_zen, bin_end, cut)
        proton.append("Proton")
        proton.append("red")

        iron = zenith_diff_corrected2(true_fe_zen, reco_fe_zen, bin_end, cut)
        iron.append("Iron")
        iron.append("blue")
        data.append([proton, iron])
...
def new_open_angle_p_fe_by_tier(mc_zen_p, reco_zen_p, mc_az_p, reco_az_p, mc_zen_fe,
                            reco_zen_fe, mc_az_fe, reco_az_fe, bin_ends, titles,
                            cuts):
    data = []
    
    for (true_prot_zen, reco_prot_zen, true_prot_az, reco_prot_az, 
         true_fe_zen, reco_fe_zen, true_fe_az, reco_fe_az, bin_end, cut) in zip(mc_zen_p, reco_zen_p, mc_az_p, reco_az_p,
                                                                               mc_zen_fe, reco_zen_fe, mc_az_fe, reco_az_fe,
                                                                               bin_ends, cuts):
        proton = open_angle_vs_zen_corrected(true_prot_zen, reco_prot_zen, true_prot_az,
                                           reco_prot_az, bin_end, cut)
        proton.append("Proton")
        proton.append("red")

        iron = open_angle_vs_zen_corrected(true_fe_zen, reco_fe_zen, true_fe_az,
                                         reco_fe_az, bin_end, cut)
        iron.append("Iron")
        iron.append("blue")
        data.append([proton, iron])
