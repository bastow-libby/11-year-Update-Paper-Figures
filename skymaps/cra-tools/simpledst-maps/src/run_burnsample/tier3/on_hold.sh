...
# Identify empty files created by the failed job
log_dir=$(condor_q -long $job_id | grep "Out" | awk '{print $3}' | xargs dirname)
if [ -d "$log_dir" ]; then
    find "$log_dir" -type f -empty -name "*$job_name*" -exec rm -f {} \;
    echo "Removed empty files related to job $job_id in $log_dir."
fi
...
echo "Job $job_id memory updated to $new_memory MB, released, and empty files cleaned up."
